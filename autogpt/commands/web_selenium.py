...
def scrape_text_with_selenium(url: str, agent: Agent) -> Tuple[WebDriver, str]:
    ...
    if agent.config.selenium_web_browser == "firefox":
        ...
        if agent.config.chromium_binary_location:
            options.binary_location = agent.config.chromium_binary_location
        if agent.config.selenium_headless:
            options.add_argument("--headless=new")
            options.add_argument("--disable-gpu")

        chromium_driver_path = Path(agent.config.chromium_driver_path)

        driver = uc.Chrome(service=ChromeDriverService(str(chromium_driver_path))
